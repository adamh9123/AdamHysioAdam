...
import { EduPackMiniLoader } from './loading-animation';
import {
  Mail,
  Download,
  Copy,
  ...
  const handlePDFDownload = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/edu-pack/${content.id}/pdf`, {
        method: 'GET',
        headers: {
          'x-edupack-content': encodeURIComponent(JSON.stringify(content))
        }
      });
      ...
      setState(prev => ({
        ...
        result: {
          success: false,
          message: error instanceof Error ? error.message : 'PDF download mislukt'
        }
      }));
    }
  }, [content, onComplete]);
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        >
          {isProcessing ? (
            <>
              <EduPackMiniLoader className="mr-2" size="sm" />
              Versturen...
            </>
          ) : (
            <>
