...
export function StandaloneForm({
  ...
  const handleDocumentUploadComplete = useCallback((documentContent: string, filename: string) => {
    updateFormData({
      uploadedDocument: { name: filename } as File, // Store filename for display
      documentContent: documentContent,
      sessionContext: documentContent.substring(0, 2000) // Limit initial context
    });
      ...
      <Card>
        ...
        <CardContent className="grid gap-4">
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <div>
              <Label>Woordenaantal</Label>
              <Select
                value={formData.wordCount}
                onValueChange={(value: 'kort' | 'middel' | 'lang') => updateFormData({ wordCount: value })}
              >
                <SelectTrigger className="mt-1">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="kort">Kort (~300 woorden)</SelectItem>
                  <SelectItem value="middel">Middel (~600 woorden)</SelectItem>
                  <SelectItem value="lang">Lang (~1000 woorden)</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div>
    ...
    <div className="space-y-4">
      <div>
        ...
        <DictationTextarea
          ...
          onChange={onSessionContextChange}
          placeholder="Beschrijf wat er tijdens de sessie is besproken: klachten, bevindingen, behandelplan, adviezen... U kunt ook de dictatiefunctie gebruiken om uw tekst in te spreken."
          rows={6}
          maxLength={5000}
...
function UploadInputSection({
  ...
  uploadedDocument?: File;
  documentContent?: string;
  onDocumentUpload: (documentContent: string, filename: string) => void;
