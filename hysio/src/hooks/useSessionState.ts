...
export type SessionType = 'intake' | 'intake-plus' | 'followup' | 'consult';
...
export const useSessionState = (options: UseSessionStateOptions = {}): UseSessionStateReturn => {
  ...
  const startSession = useCallback((type: SessionType, patientInfo: PatientInfo) => {
    ...
    // Determine if this is an intake-type session
    const isIntakeType = type === 'intake' || type === 'intake-plus';
    const isFollowupType = type === 'followup' || type === 'consult';
    ...
    const newSession: SessionState = {
      ...
      currentStep: isIntakeType ? 'preparation' : 'session-planning',
      startedAt: now,
      pausedAt: null,
      completedAt: null,
      lastSavedAt: null,
