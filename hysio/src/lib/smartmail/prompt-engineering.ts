...
export function generateSystemPrompt(
  ...
  // Add enhanced document integration guidance if applicable
  if (includeDocuments) {
    const documentGuidance = language === 'nl'
      ? `\n\nGeavanceerde Documentintegratie:
- Analyseer zorgvuldig alle bijgeleverde documenten en hun metadata
- Gebruik specifieke details, bevindingen, en medische informatie uit de documenten
- Verwijs alleen naar informatie die expliciet in de documenten staat
- Citeer relevante delen wanneer nodig voor medische nauwkeurigheid
- Let op document waarschuwingen en beperkingen (bijv. PDF extractie kwaliteit)
- Combineer document informatie intelligent met de handmatig ingevoerde context
- Vermeld documentbronnen waar relevant voor transparantie`
      : `\n\nAdvanced Document Integration:
- Carefully analyze all provided documents and their metadata
- Use specific details, findings, and medical information from documents
- Only reference information explicitly present in the documents
- Quote relevant sections when needed for medical accuracy
- Note document warnings and limitations (e.g., PDF extraction quality)
- Intelligently combine document information with manual context
- Reference document sources where relevant for transparency`;
...
export function generateUserPrompt(
  ...
  // Add enhanced document content if provided
  if (documentContent && documentContent.length > 0) {
    userPrompt += `**${language === 'nl' ? 'Bijgeleverde documenten voor analyse' : 'Provided documents for analysis'}:**\n`;
    userPrompt += `${language === 'nl'
      ? 'Let op: Gebruik alleen informatie die expliciet in deze documenten staat. Respecteer eventuele extractie waarschuwingen.'
      : 'Note: Only use information explicitly present in these documents. Respect any extraction warnings.'}\n\n`;
    ...
    userPrompt += `${language === 'nl'
      ? '=== Einde documenten ===\n\nIntegreer de bovenstaande documentinformatie intelligent in uw email response.'
      : '=== End of documents ===\n\nIntelligently integrate the above document information into your email response.'}\n\n`;
  }
