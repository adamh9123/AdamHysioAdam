...
// Enhanced document content processing with improved extraction
async function processDocuments(documents: DocumentContext[]): Promise<{
  ...
  for (const doc of documents) {
    try {
      // Convert DocumentContext to format expected by extractor
      const fileData = {
        name: doc.filename,
      ...
      const result = await DocumentContentExtractor.extractContent(fileData, {
        maxLength: 10000, // Limit for AI processing
      ...
      if (result.success) {
        ...
        const validation = DocumentContentExtractor.validateForAIProcessing(result);
        if (!validation.isValid) {
          warnings.push(`Document ${doc.filename}: ${validation.issues.join(', ')}`);
        }

        // Add extraction warnings
        if (result.warnings) {
          warnings.push(...result.warnings.map(w => `${doc.filename}: ${w}`));
        }

        // Format for AI prompt
        const formatted = DocumentContentExtractor.formatForAIPrompt(result, true);
        formattedContent.push(formatted);
      } else {
        warnings.push(`Failed to process ${doc.filename}: ${result.error}`);
        // Fallback to original content
        formattedContent.push(`**${doc.filename}** (${doc.source}):\n${doc.content}`);
      }
    } catch (error) {
      warnings.push(`Error processing ${doc.filename}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      // Fallback to original content
      formattedContent.push(`**${doc.filename}** (${doc.source}):\n${doc.content}`);
    }
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const documentProcessing = documents.length > 0
      ? await processDocuments(documents)
      : { formattedContent: [], extractionResults: [], warnings: [] };
    ...
    const documentWarnings = documentProcessing.warnings.map(warning => ({
      type: 'document_processing' as const,
      message: warning,
      severity: 'warning' as const,
      actionable: true,
      suggestedAction: 'Check document content and consider providing manual text input'
    }));
    const allSuggestions = [...contentWarnings, ...contextualSuggestions, ...documentWarnings];

    // Use processed document content
    const documentContent = documentProcessing.formattedContent.length > 0
      ? documentProcessing.formattedContent
      : undefined;
    ...
    const openAIOptions: OpenAICompletionOptions = {
      model: HYSIO_LLM_MODEL, // Use GPT-4.1-mini for healthcare communication
      temperature: 0.7, // Balanced for professional healthcare communication
      max_tokens: 2000, // Sufficient tokens for comprehensive emails
      top_p: 0.9,
