 ...
 * Matches the format generated by nanoid(21) in /pre-intake/page.tsx
 */
const sessionIdSchema = z
  .string()
  .min(21, 'Session ID must be at least 21 characters')
  .max(21, 'Session ID must be at most 21 characters')
  .regex(/^[A-Za-z0-9_-]{21}$/, 'Invalid session ID format');
  ...
  try {
    ...
    if (!validation.success) {
      return NextResponse.json(
        {
          success: false,
          error: 'Invalid session ID format. Must be a 21-character nanoid.',
        },
        { status: 400 }
      );
    }
